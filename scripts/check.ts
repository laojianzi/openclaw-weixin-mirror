...
export async function checkForUnsyncedVersions(
	params: CheckParams,
): Promise<string[]> {
	...
	try {
		existingReleases = await listReleases(
			params.githubToken,
			params.repository,
		);
	} catch (err: unknown) {
	...
	if (params.githubOutput) {
		appendFileSync(
			params.githubOutput,
			`unsynced_count=${unsyncedVersions.length}\n`,
		);
		appendFileSync(
			params.githubOutput,
			`unsynced_versions=${unsyncedVersions.join(",")}\n`,
		);
	}
